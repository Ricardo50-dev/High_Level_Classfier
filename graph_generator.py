...
def rede_eng_Graph(raio, X, y, med, dist):
    ...
    for l in range(numclasses):
        ...
        if med == 'Assortativity':
            measures[l] = Assortativity(subg)
        elif med == 'ClustCoefficient':
            measures[l] = ClustCoefficient(subg)
        elif med == 'AvgDegree':
            measures[l] = AvgDegree(subg)
        elif med == 'Betweenness':
            measures[l] = Betweenness(subg)
        elif med == 'AvgPathLength':
            measures[l] = AvgPathLength(subg)
        elif med == 'Closeness':
            measures[l] = Closeness(subg)
...
def VS_Graph_vector(k, X, y, med, dist, typ):
    ...
    if dist == 'cosine':
        dist_matrix = pairwise.cosine_similarity(centrality)
    else:
        dist_metric = DistanceMetric.get_metric(dist)
        dist_matrix = dist_metric.pairwise(centrality)

    np.fill_diagonal(dist_matrix, np.inf)

    # Seleciona os k vizinhos mais próximos
    ind_ranking = np.argsort(dist_matrix, axis=1)[:, :k]

    # Criação da máscara de links
    mask = (y[ind_ranking] == y[:, None])
    sources, col_indices = np.where(mask)
    targets = ind_ranking[sources, col_indices]

    # Inicializa variáveis para armazenar os grafos e medidas
    lista_grafos = []
    measures = np.zeros(num_classes)
    map_vertices = np.full(n_samples, -1, dtype=int)

    # Dicionário para cálculo de medidas de rede
    measure_functions = {
        'Assortativity': Assortativity,
        'ClustCoefficient': ClustCoefficient,
        'AvgDegree': AvgDegree,
    ...
    if med not in measure_functions:
        raise ValueError(f"Medida '{med}' não reconhecida.")

    # Construção dos grafos por classe
    for l in unique_classes:
        # Seleciona os objetos da classe l
        class_mask = (y[sources] == l)
        lsources = sources[class_mask]
        ltargets = targets[class_mask]
        all_vertices = np.where(y == l)[0]

        # Obtém vértices únicos da classe l
        unique_vertices = np.unique(np.concatenate((lsources, ltargets, all_vertices)))

        # Mapeia os índices dos vértices
        map_vertices[unique_vertices] = np.arange(len(unique_vertices))

        # Cria o grafo para a classe l
        subg = ig.Graph(
            n=len(unique_vertices),
            edges=list(zip(map_vertices[lsources], map_vertices[ltargets]))
        )

        lista_grafos.append(subg)

        # Calcula a medida correspondente
        measures[l] = measure_functions[med](subg)
